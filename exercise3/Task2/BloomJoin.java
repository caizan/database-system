...
class NodeA extends Node {
    ...
    public void run(Node other) {
        ...
        data.forEach(r -> bloomFilter.set(hashFunction(r.getB()),true));
        send(other,bloomFilter);
        other.run(this);
        Message msg = this.getInbox();
        List<S> BData = (List<S>) msg.getContent();
...
class NodeB extends Node {
    ...
    public void run(Node other) {
        Message msg = this.getInbox();
        BitSet bloomFilter =(BitSet) msg.getContent();
        List<S> toReturn = new ArrayList<>();
        for (S s : data) {
            if (bloomFilter.get(hashFunction(s.getB())))
                toReturn.add(s);
        }
        send(other,toReturn);
    }
